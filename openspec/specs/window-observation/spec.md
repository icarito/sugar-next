...
Define how Sugar Next observes window open/close/focus state — for
windows it launches and for windows opened outside it — across its two
startup modes (see `shell-startup`). Two adapters feed one internal event
contract: a GNOME Shell extension over D-Bus in hosted mode, and the
`wlr-foreign-toplevel-management` protocol in standalone mode. Neither
adapter gives Sugar Next ownership of window placement; both only report
state.
